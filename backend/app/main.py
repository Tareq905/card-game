...
from .models import User, Match, ThemeSong, PaymentTransaction, Report, Achievement, SongRequest, BanLog
...
async def get_match_history(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        ...
        for m in all_matches:
            ...
            if any(p.get("id") == current_user.id for p in players):
...
SYSTEM_PROMPT = """You are the One Left assistant for the card game "One Left". Your job is STRICTLY limited to answering questions about:
