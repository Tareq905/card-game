...
import { Trophy, Medal, Star, ArrowLeft, User, Target, Zap, Shield, Crown, Pencil, Check, X } from 'lucide-react';
...
export default function Profile() {
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      form.append('display_name', trimmed);
      ...
      if (!res.ok) {
        ...
        setNameError(d.detail || 'Failed to save.');
        return;
      }
                      ...
                      <div style={{ marginBottom: '8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <input
                            ...
                            maxLength={30}
                            style={{
                              fontSize: '20px', fontWeight: 700, padding: '6px 10px', borderRadius: '8px',
                              background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(139,92,246,0.5)',
                          ...
                          <button
                            ...
                            disabled={savingName}
                            style={{ background: 'var(--accent-green)', border: 'none', borderRadius: '8px', padding: '6px 10px', cursor: 'pointer', color: 'white', display: 'flex', alignItems: 'center' }}
                          >
                            <Check size={16} />
                          </button>
                          <button
                            onClick={() => { setEditingName(false); setNameError(''); }}
                            style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '8px', padding: '6px 10px', cursor: 'pointer', color: 'white', display: 'flex', alignItems: 'center' }}
                          >
                            <X size={16} />
                          </button>
                        </div>
                        {nameError && <div style={{ color: 'var(--accent-red)', fontSize: '12px', marginTop: '4px' }}>{nameError}</div>}
                      </div>
                    ) : (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px' }}>
                        <h2 style={{ fontSize: '24px', fontWeight: 800, margin: 0 }}>{profile.display_name || profile.email}</h2>
                        <button
                          onClick={() => { setNameInput(profile.display_name || ''); setEditingName(true); setNameError(''); }}
                          title="Edit display name"
                          style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '6px', padding: '4px 6px', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center' }}
                        >
                          <Pencil size={13} />
                        </button>
                      </div>
