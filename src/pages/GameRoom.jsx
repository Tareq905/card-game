      ...
      <div style={{
        ...
        {otherPlayers.map((player) => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '150px',
              border: isPlayerTurn ? '1px solid var(--accent-blue)' : '1px solid var(--bg-panel-border)',
              boxShadow: isPlayerTurn ? '0 0 15px rgba(59, 130, 246, 0.3)' : 'none',
              transition: 'all 0.3s ease', opacity: player.is_active ? 1 : 0.4,
              padding: '14px 20px', borderRadius: '12px'
            }} className="glass-panel">
              <span style={{ fontSize: '13px', fontWeight: 600, color: isPlayerTurn ? 'var(--accent-blue)' : '#fff' }}>
                {player.phone} {!player.is_active && '(DC)'}
              </span>
              <span style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px' }}>
                {player.card_count} cards
              </span>

              {/* One Left Status and CATCH button */}
              {hasOneCard && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '10px', width: '100%' }}>
                  <span style={{
                    fontSize: '11px', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', textAlign: 'center',
                    background: yelled ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                    color: yelled ? 'var(--accent-green)' : 'var(--accent-red)'
                  }}>
                    {yelled ? 'ONE LEFT YELLED!' : 'ONE CARD LEFT!'}
                  </span>
                  {!yelled && (
                    <button onClick={() => handleCatch(player.id)} className="btn-danger"
                      style={{ padding: '6px', fontSize: '11px', borderRadius: '6px', width: '100%', fontWeight: 700 }}>
                      CATCH THEM!
                    </button>
                  )}
                </div>
              ...
              {!player.is_bot && (
                <button
                  ...
                  style={{ marginTop: '8px', background: 'none', border: 'none', color: 'rgba(239,68,68,0.5)', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px' }}
                >
                  <ShieldX size={12} /> Report
                </button>
              )}
            </div>
          );
